...
p_grhs' parentPlacement placer render style (GRHS _ guards body) =
  case guards of
    ...
    xs -> do
      txt "|"
      space
      alignContext $ sitccIfTrailing (sep commaDel (sitcc . located' p_stmt) xs)
      align
      inci $ case style of
        EqualSign -> equals
...
p_stmt' placer render = \case
  ...
  RecStmt {..} -> do
    txt "rec"
    space
    alignContext . sitcc . located recS_stmts $ sepSemi (withSpacing (p_stmt' placer render))
